...
function [ scaled_image ] = resizeNearest( img, scale, gray )
%RESIZENEAREST Resizes a picture using nearest neighbour interpolation
...
for i=1:scale:scaledim_size(1)
    for j=1:scaledim_size(2)
        pos=mod(j-1,scale);
        if pos>scale/2
            scaled_image(i,j,:)=scaled_image(i,j-pos+scale,:);
        else if pos~=0
            scaled_image(i,j,:)=scaled_image(i,j-pos,:);
            end
        end
    end
end
...
for i=1:scaledim_size(1)
    ...
    pos=mod(i-1,scale);
        if pos>scale/2
            scaled_image(i,j,:)=scaled_image(i-pos+scale,j,:);
        else if pos~=0
            scaled_image(i,j,:)=scaled_image(i-pos,j,:);
            end
        end
